...
export function Grid({ className, ...rest }: HTMLAttributes<HTMLDivElement>): ReactNode {
  ...
  return (
    <div
      ...
      // }}
      className={cn('absolute inset-0 z-0 flex size-full overflow-hidden', className)}
      {...rest}
    >
      {rows.map((_, i) => (
        <motion.div key={'row' + i} className='size-8 border-l border-muted-foreground/20'>
          {cols.map((_, j) => (
            <motion.div
              whileHover={{
