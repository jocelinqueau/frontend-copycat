...
const getDominantColors = async (
  ...
  for (let i = 0; i < 10; i++) {
    const distances = tf
      .sub(inputFlat.expandDims(1), centroids.expandDims(0))
      .square()
      .sum(2);
    ...
    <div className="flex h-screen w-full flex-col items-center justify-center gap-4">
      ...
      {file && (
        <div className="rounded-sm overflow-hidden relative">
          <div
            className="absolute top-2 right-2 text-muted opacity-50 cursor-pointer hover:opacity-100"
            onClick={() => {
              setFile(null);
              setHexasColors([]);
              setSelectedColor("");
          ...
          <div className="flex flex-row items-center justify-center gap-2">
            {hexasColors.map((color, index) => (
              <div
                ...
                key={index}
                className="w-8 h-8 border border-gray-200 opacity-50 hover:opacity-100 rounded-sm cursor-pointer"
